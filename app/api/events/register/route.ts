...
export async function POST(req: NextRequest) {
  ...
  if (website) { console.warn("[events/register] honeypot activado (campo 'website' relleno; posible autofill del navegador) — email:", email); return NextResponse.json({ ok: true }); } // honeypot
  ...
  if (process.env.NEXT_PUBLIC_SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY) {
    try {
      ...
      if (insErr) console.error("[events/register] supabase insert FAILED:", insErr.message, "| email:", email, "| event:", event_name);
    } catch (err) {
