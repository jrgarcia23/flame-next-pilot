...
export async function DELETE(req: NextRequest) {
  ...
  if (!Array.isArray(body.ids) || body.ids.length === 0) return NextResponse.json({ error: "Falta lista de ids" }, { status: 400 });
  if (body.ids.length > 500) return NextResponse.json({ error: "Demasiados ids (máx 500)" }, { status: 400 });
  ...
  const { error, count } = await supabase.from("webinar_registrations").delete({ count: "exact" }).in("id", ids);
  if (error) {
    console.error("[admin/webinars DELETE]", error);
