  ...
  return (
    <div style={wrap}>
      ...
      <div style={inner}>
        ...
        <p style={{ fontSize: 13.5, color: "#6E7488", margin: "0 0 20px" }}>Sector, producto y caso de uso de cada caso. Filtra, ordena y edita; los cambios se guardan al instante.</p>
        {error ? (
          <div style={{ background: "#FFF3D6", border: "1px solid #F5D98B", borderRadius: 12, padding: "20px 24px", fontSize: 14, lineHeight: 1.6, color: "#7a4f00" }}>
            <b>Falta crear la tabla en Supabase.</b> Ejecuta una vez este SQL en el editor SQL de Supabase (proyecto Flame) y recarga:
            <pre style={{ marginTop: 12, padding: 14, background: "#15163A", color: "#dbe4ff", borderRadius: 8, overflowX: "auto", fontSize: 12 }}>{`create table if not exists public.case_study_tags (
...
create policy "case_study_tags_read" on public.case_study_tags for select using (true);`}</pre>
            ...
            <div style={{ marginTop: 10, fontSize: 12, color: "#94793a" }}>Detalle: {error.message}</div>
          </div>
        ) : (
          <CasosTagsTable rows={(data || []) as TagRow[]} vocab={{ sector: SECTORS, productos: PRODUCTOS, casosDeUso: USOS }} />
        )}
      </div>
    </div>
  );
