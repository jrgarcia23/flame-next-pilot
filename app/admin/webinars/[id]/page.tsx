...
export default async function WebinarDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!email || !isEmailAllowed(email)) redirect(`/admin/login/?next=${encodeURIComponent(`/admin/webinars/${id}/`)}`);

  const supabase = createSupabaseAdminClient();
  const { data, error } = await supabase.from("webinar_registrations").select("*").eq("id", parseInt(id)).single();
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F6F7FB", fontFamily: '-apple-system, "Segoe UI", "Inter", sans-serif', color: "#15163A" }}>
      ...
      <div style={{ maxWidth: 1080, margin: "0 auto", padding: "28px 32px" }}>
        <Link href="/admin/webinars/" style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 12, fontWeight: 500, color: "#6E7488", textDecoration: "none", marginBottom: 12, padding: "4px 8px", borderRadius: 6 }}>
        ...
        <p style={{ fontSize: 12, color: "#6E7488", margin: "0 0 4px 0" }}>
          <Link href="/admin/webinars/" style={{ color: "#6E7488", textDecoration: "none" }}>Inicio · Webinars</Link> · #{reg.id}
        </p>
        ...
        <div style={{ background: "#fff", border: "1px solid rgba(15,23,42,0.08)", borderRadius: 12, overflow: "hidden", marginTop: 12 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 0 }} className="lead-grid">
            <div style={{ padding: "24px 28px", borderRight: "1px solid rgba(15,23,42,0.08)" }}>
              ...
              <h4 style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", color: accentDeep, margin: "24px 0 12px 0" }}>Webinar</h4>
              <dl style={{ display: "grid", gridTemplateColumns: "120px 1fr", gap: "10px 16px", fontSize: 13, margin: 0 }}>
                <Row label="Nombre" value={reg.webinar_name} />
                <Row label="Fecha" value={reg.webinar_date ? fmt(reg.webinar_date) : null} />
              </dl>
            </div>
