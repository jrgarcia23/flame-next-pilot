...
export const metadata: Metadata = {
  ...
  twitter: {
    ...
    title: "Conteo de personas en retail · Flame Analytics",
    description: "Mide afluencia, conversión visita-venta y staff-to-traffic en cada tienda.",
    images: [HERO_BG],
  },
};
...
const BENEFITS: Benefit[] = [
  { icon: "eye",      title: "Visibilidad real de cada tienda",   desc: "Sabes cuántas personas visitan cada punto de venta, cuándo y cómo evolucionan las tendencias. No medias engañosas: dato tienda a tienda." },
  { icon: "users",    title: "Plantilla y operaciones por dato",  desc: "Ajusta turnos y dotación a la afluencia real de cada tienda. Reduce horas mal asignadas y mejora la atención en hora punta." },
  { icon: "grid",     title: "Compara y prioriza tiendas",        desc: "Identifica las tiendas top en conversión visita-venta y replica sus prácticas. Detecta las que rinden por debajo de su potencial." },
  { icon: "trending", title: "Conexión con el TPV",                desc: "Cruza visitantes con tickets de venta para descubrir oportunidades reales de conversión y ROI por tienda." },
];

const METRICS: Metric[] = [
  { icon: "clock",    title: "Afluencia total y por franja",  desc: "Visitas reales día a día, hora a hora. Detecta picos y valles del tráfico real en cada tienda." },
  { icon: "convert",  title: "Conversión visita → venta",     desc: "Cruza tráfico con tu TPV. Sabes qué % de visitantes acaba comprando en cada punto de venta." },
  { icon: "calendar", title: "Patrones semanales y picos",    desc: "Identifica los momentos críticos de cada tienda. Anticipa con personal y stock." },
  { icon: "compare",  title: "Benchmark entre tiendas",       desc: "Compara red completa con criterio homogéneo. Identifica top y replica sus prácticas." },
  { icon: "street",   title: "Tráfico exterior y captación",  desc: "Cuántas personas pasan frente a tu tienda y cuántas entran. Mide la capacidad del escaparate." },
...
const DEPARTMENTS: Department[] = [
  {
    icon: "trending",
    title: "¿Eres de Marketing retail?",
    desc: "Conecta inversión en campañas con flujo real en tienda. Atribuye visitas a punto de venta, mide retorno por canal y activa acciones según comportamiento del visitante.",
    bulletsLabel: "KPIs clave",
    bullets: [
      "Captación: tráfico exterior vs entradas",
      "Atribución de campañas digitales a tienda física",
      "Customer journey por punto de venta",
      "Triggers de campañas en tiempo real",
    ],
  },
  {
    icon: "compare",
    title: "¿Eres de Operaciones de tienda?",
    desc: "Decide plantilla, horarios y aperturas con datos reales por tienda. Compara conversión visita-venta de cada punto de venta y prioriza donde el ratio no encaja con el potencial.",
    bulletsLabel: "KPIs clave",
    bullets: [
      "Staff-to-traffic por turno",
      "Conversión visita-venta por tienda",
      "Ranking de tiendas y benchmarks",
      "Picos horarios y dotación óptima",
    ],
  },
  {
    icon: "grid",
    title: "¿Eres del equipo IT retail?",
    desc: "Aprovecha las cámaras CCTV ya instaladas en cada tienda. Sin hardware nuevo, sin cableado, sin proyecto de despliegue. Implementación remota en toda la red retail.",
...
const HERO_BULLETS = ["Precisión 99 %", "+500 tiendas medidas", "Sin biometría", "Compatible con tu CCTV"];

export default function ConteoPersonasRetail() {
  const currentLang: "es" | "en" = "es";
  const t = UI[currentLang];
  const faqs = getFaqs("retail", "es");
  ...
  return (
    <>
        ...
        <div className="flame-container">
          <div className="text-center mb-14 mx-auto" style={{ maxWidth: 760 }}>
            <h2 className="text-[clamp(32px,3.4vw,48px)] font-normal" style={{ color: "var(--color-navy)", letterSpacing: "-0.02em", lineHeight: 1.08, fontFamily: "var(--font-display)" }}>
              <span style={{ color: "var(--color-accent)", fontWeight: 500 }}>Beneficios</span> del conteo de personas en retail
            </h2>
            <p className="mt-5 text-[clamp(17px,1.25vw,19px)] leading-relaxed" style={{ color: "var(--color-ink-2)" }}>
              Cada métrica convierte tu intuición en decisión: planifica plantilla por tienda, mide conversión por punto de venta y compara la red completa con dato real.
            </p>
          </div>
      ...
      <section className="py-24" style={{ background: "var(--color-paper)" }}>
        <div className="flame-container">
          <div className="text-center mx-auto mb-14" style={{ maxWidth: "44ch" }}>
            <h2 className="text-[clamp(32px,3.4vw,48px)] font-normal" style={{ color: "var(--color-navy)", letterSpacing: "-0.02em", lineHeight: 1.08, fontFamily: "var(--font-display)" }}>
              Más allá del conteo: inteligencia <span style={{ color: "var(--color-accent)", fontWeight: 500 }}>retail para cada tienda</span>
            </h2>
          </div>
          ...
          <div style={{ marginTop: 64, paddingTop: 48, borderTop: "1px solid rgb(21 22 58 / 0.08)" }}>
            <h3 className="text-[clamp(24px,2.4vw,32px)] font-normal" style={{ color: "var(--color-navy)", letterSpacing: "-0.018em", lineHeight: 1.15, fontFamily: "var(--font-display)", marginBottom: 10 }}>
              Las métricas que <span style={{ color: "var(--color-accent)", fontWeight: 500 }}>cambian decisiones</span>
            </h3>
            <p style={{ fontSize: 16, color: "var(--color-ink-2)", lineHeight: 1.6, marginBottom: 32, maxWidth: 620 }}>
              Seis indicadores que convierten tu intuición en decisión operativa retail.
            </p>
      ...
      {/* CTA STRIP NAVY — encima de Departamentos */}
        ...
        <div className="flame-container">
          <div className="flex items-center gap-8 cta-strip-row">
            <p className="text-[clamp(19px,1.55vw,24px)] font-medium flex-1" style={{ color: "#fff", fontFamily: "var(--font-body)", letterSpacing: "-0.005em", lineHeight: 1.35 }}>
              Cada tienda es única. Tu data debe demostrarlo.<br />
      ...
      {/* DEPARTAMENTOS RETAIL */}
      <section className="py-24" style={{ background: "#fff" }}>
        <div className="flame-container">
          <div className="mb-14 mx-auto text-center" style={{ maxWidth: 820 }}>
            <h2 className="text-[clamp(32px,3.4vw,48px)] font-normal" style={{ color: "var(--color-navy)", letterSpacing: "-0.02em", lineHeight: 1.08, fontFamily: "var(--font-display)" }}>
              Pensado para <span style={{ color: "var(--color-accent)", fontWeight: 500 }}>cualquier área</span> de tu retail
            </h2>
            <p className="mt-4 text-[clamp(16px,1.15vw,18px)] leading-relaxed" style={{ color: "var(--color-ink-2)" }}>
              Marketing, operaciones de tienda o IT: Flame entrega los KPIs que cada equipo retail necesita para decidir mejor. En la misma plataforma.
            </p>
          </div>
          <div className="grid gap-5 dept-grid" style={{ gridTemplateColumns: "repeat(3, minmax(0, 1fr))", alignItems: "stretch" }}>
            {DEPARTMENTS.map((d, i) => (
              <article key={i} className="rounded-2xl p-7 flex flex-col" style={{ background: "var(--color-paper)", border: "1px solid var(--color-rule)" }}>
